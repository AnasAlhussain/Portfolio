...
const About = ({ setPage }: Props) => {
  ...
  const downloadCV = () => {
    ...
    if (!fileId) return;
    const url = `https://drive.google.com/file/d/1zYQ3OZU9XPu_LrXCb7qXbkHZuRtLWr9B/view?usp=drive_link`;
   ...
   const downloadCVArabic = () => {
    setLoading(true);
    const fileId = extractDriveFileId(aboutMe.cvDriveLink);
    if (!fileId) return;
    const url = `https://drive.google.com/file/d/1iN3mKVYw1e5M-cS6dkPla_NQvLDIwp41/view?usp=drive_link`;
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "cv.pdf");
    document.body.appendChild(link);
  ...
  return (
    ...
    >
      ...
      >
        <FaDownload />
        <Text marginLeft={2}>Download CV In Arabic</Text>
      </Button>
